...
#def clean_url(data):
#    extractor = URLExtract()
#    urls = data['query'].apply(lambda x: ' '.join([url for url in extractor.find_urls(x) if url.endswith('.local')]) if extractor.find_urls(x) else np.nan)
#    return urls
...
# loaded model
    log_file_path = "/opt/zeek/spool/zeek/dns.log"

    with subprocess.Popen(['tail', '-f', log_file_path], stdout=subprocess.PIPE, bufsize=1, universal_newlines=True) as proc:
        for line in proc.stdout:
            ...
            if line and line[0] != '#':
               ...
               # 
                ...
                dns_log['attack'] = y_predict
                print('Model prediction Completed')
                dns_log.to_csv('Prediction.csv')
