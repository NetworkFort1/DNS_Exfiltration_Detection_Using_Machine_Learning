            ...
            # for line in proc.stdout:
                ...
                if line and line[0] != '#':
                    ...
                    if len(queries) < 20:
                        ...
                        dns_log = pd.DataFrame({'ts': timestamps, 'query': queries})
                        ...
                        if dns_log.empty or dns_log.shape[0] < 10:
                            timestamps = []
                            queries = []
                            continue
                        ...
                        # print(dns_log_copy)
                        records = dns_log_copy.to_dict('records')

                        for record in records:
                            if record["attack"]:
                                es.index(index="dns-exfiltration-alerts", document=record)
